...
public class UserController {
    ...
    public ResponseEntity<Users> addUsers(@Valid @RequestBody Users users) {
	 ...
	 @PostMapping("/Enquiry/{cid}/{pid}")
	    public String addCourseEnquiryMapDTO(@RequestBody CouseEnquriymapDTO courseEnquiryMapDTO, @PathVariable("cid") int cid, @PathVariable("pid") int pid) {
	        try {
	            // Get the user ID from an external service
	            ResponseEntity<Long> userIdResponse = restTemplate.getForEntity("http://192.168.1.111"
	            		+ ":8080/api/test/tokenusername", Long.class);
	            ...
	            // ResponseEntity<Integer> pipelineIdResponse = restTemplate.getForEntity("http://192.168.1:8083/phases/{pid}/", Integer.class);
	            // int pipelineId = pipelineIdResponse.getBody();
	            //
	            // ResponseEntity<Integer> courseTypeIdResponse = restTemplate.getForEntity("http://192.168.1:8083/Courses/{cid}", Integer.class);
	            ...
	            // if (enquiryDto.getCourseId() == courseTypeId && enquiryDto.getPipeLinePhaseId() == pipelineId) {
	            
	            // Send the EnquiryDto to the external service
	            restTemplate.postForEntity("http://192.168.1.110:8083/enquiries/add/" + pid + "/" + cid, enquiryDto, EnquiryDto.class);
	            // Send the ActivityDto to the external service
	            restTemplate.postForEntity("http://192.168.1.116:8082/api/activities/createActivity", activityDto, Activitydto.class);
