...
public class WeeksController {
	...
	public ResponseEntity<?> createWeekDay(@Valid @RequestBody WeekDays weekDay) {
		try {
			...
			return new ResponseEntity<>(
					createResponse("WeekDay created successfully", HttpStatus.CREATED, createdWeekDay),
					HttpStatus.CREATED);
		} catch (Exception e) {
	...
	public ResponseEntity<?> updateWeekDay(@PathVariable Integer id, @Valid @RequestBody WeekDays weekDay) {
		try {
			...
			return new ResponseEntity<>(createResponse("WeekDay updated successfully", HttpStatus.OK, updatedWeekDay),
					HttpStatus.OK);
		} catch (DataNotFoundException e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND)
	...
	@GetMapping("weekday")
	public Optional<WeekDays> getMethodName() {
		return weekRepository.findByWeekDayName("Wednesday");
	}
